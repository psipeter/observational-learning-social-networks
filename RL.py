...
def run_RL(sid, z, k, learning_rate, save=True):
    ...
    for trial in trials:
        ...
        for stage in range(4):
            ...
            df = pd.DataFrame([['human', sid, trial, stage, action_emp, None, None, None, None]], columns=columns)
            dfs.append(df)
            df = pd.DataFrame([['model-RL', sid, trial, stage, action_sim, error, z, k, learning_rate]], columns=columns)
            dfs.append(df)
