...
if __name__ == '__main__':
	...
	sid = int(sys.argv[3])
	if dataset=='jiang' and model_type=='NEF_syn':
		...
		start = time.time()
		activity_data = activities_NEF_syn(dataset=dataset, sid=sid, alpha=alpha, z=z, lambd=lambd, n_neurons=n_neurons)
		print(activity_data)
		activity_data.to_pickle(f"data/{dataset}_{model_type}_{sid}_activities.pkl")
		end = time.time()
		print(f"runtime {(end-start)/60:.4} min")
	if dataset=='yoo' and model_type=='NEF_syn':
		p_nef = pd.read_pickle("data/NEF_syn_yoo_apr9_params.pkl").query("sid==@sid")
		n_neurons = p_nef['n_neurons'].unique()[0]
		alpha = p_nef['alpha'].unique()[0]
		lambd = p_nef['lambda'].unique()[0]
		start = time.time()
		activity_data = activities_NEF_syn(dataset=dataset, sid=sid, alpha=alpha, z=0, lambd=lambd, n_neurons=n_neurons)
		print(activity_data)
		activity_data.to_pickle(f"data/{dataset}_{model_type}_{sid}_activities.pkl")
		end = time.time()
		print(f"runtime {(end-start)/60:.4} min")
