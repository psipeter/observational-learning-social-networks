...
def simulate_NEF_rec(learned_weights, env, n_neurons=1000, seed_net=0, syn=0.01, syn_fb=0.2,
...
def run_NEF_rec(dataset, sid, alpha, z, lambd, n_neurons=200):
	...
	for trial in trials[:50]:
		...
		W = simulate_NEF_rec(W, env, alpha=alpha, n_neurons=n_neurons, lambd=lambd, seed_net=sid, train=True)
	...
	for trial in trials:
		...
		for stage in env.stages:
			if dataset=='carrabin':
				...
				df = pd.DataFrame([['NEF_rec', sid, trial, stage, estimate]], columns=columns)
				dfs.append(df)
